...
class PlaySound:
  def __init__(self, output_device_name = "Headphones (High Definition Aud, MME (0 in, 2 out)") -> None:
  ...
  def _search_output_device_id(self, output_device_name, output_device_host_api=0)->int:
    ...
    # 指定された出力デバイスとホストAPIに合致するデバイスIDを検索
    for device in devices:
      ...
      if is_output_device_host_api and is_output_device_name:
        output_device_id = device["index"]
        break

    # 合致するデバイスIDが見つからなかった場合の処理
    if output_device_id is None:
      print("output_deviceが見つかりませんでした：" + output_device_name)
      exit()
